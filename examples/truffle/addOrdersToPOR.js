...
module.exports = async (callback) => {
  ...
  stdlog(`Staked ${web3.utils.fromWei(KNCStake)} to OrderbookReserve (${OrderbookReserveInstance.address})`);

  // Deposit ETH to POR
  await OrderbookReserveInstance.depositEther(
    userWallet,
    { from: userWallet, value: web3.utils.toWei(new BN(ETHAmount)) },
  );
  ...
  stdlog(`Deposited ${web3.utils.fromWei(SNTAmount)} tokens to OrderbookReserve (${OrderbookReserveInstance.address})`);
